...
### 템플릿 메서드 패턴이란? 
- 알고리즘의 구조 (뼈대)를 정의
  - 여러 클래스에서 공통으로 사용하는 메서드를 템플릿(template)화 하여 상위 클래스에 정의 
- 일부 단계는 서브 클래스에서 구체적으로 구현할 수 있게 함 
  - 서브 클래스마다 세부 동작을 다르게 구현 
- 알고리즘의 구조를 유지한 채로 행동을 다르게 변경할 수 있다. 

### 필요 상황 예시 
- 회사 문서들을 분석하는 앱을 만들고 있다고 가정 
  - 다양한 포맷(pdf,doc,csv) 문서에 대해 _**일관된 형식으로**_ 의미있는 데이터 추출 
- 포맷에 맞게 처리 하는 부분 외에 ***많은 코드 중복***이 발생 

### 아이디어
- 알고리즘을 일련의 단계들로 나누고 이러한 단계를 메서드로 변환 
- 단일 템플릿 메서드 내부에 ***단계 메서드들에 대한 일련의 호출***로 구성 
- 상속을 통해 _**추상 단계 메서드들은 오버라이드 해서**_ 구현

![img.png](TemplateMethod/TemplateMethodIdea.png)
#### 템플릿 메서드 
  - 알고리즘의 뼈대를 이루는 메서드로 아래의 **_단계 메서드로 구성_** 
#### 단계 메서드 
  - 추상 단계 메서드 
    - 모든 ***자식 클래스에서 다르게 구현***해야 하는 메서드 
    - 디폴트 단계 메서드
      - 부모 클래스에서 기본 구현이 있어서 _**공통적으로 쓰일 수 있는**_ 메서드 
      - 필요한 경우 디폴트 구현을 무시하고, 자식 클래스에서 오버라이딩 할 수 있다. 

### 구조 
![img.png](TemplateMethod/TemplateMethodStructure.png)

### Hook Method 
- _**몸체가 비어 있는**_ 단계 메서드
- 자식 클래스가 ***선택적으로 오버라이***딩 
- 템플릿 메서드는 hook method가 _**오버라이딩 되지 않더라도 작동**_ 
- 알고리즘의 _**전/후에**_ 배치됨 
  - 자식 클래스들에게 알고리즘의 ***추가 확장 지점***을 제공 

### 장점 
- 클라이언트가 대규모의 알고리즘의 _**특정 부분(추상 단계 메소드)만 재정의 하도록**_ 한다. 
  - 알고리즘의 다른 부분에 발생하는 _**변경에 영향을 덜 받음**_ 
- 상위 클래스로 _**로직을 공통화**_ 
  - 코드의 중복 감소 
  - 핵심 로직을 ***상위 클래스에서 관리***하므로 관리가 용이 

### 단점 
- 제공되는 뼈대(템플릿)로 인해 알고리즘 ***로직의 유연성이 감소***될 수 있음 
- 하위 클래스에서 추상 단계 메서드를 구현할 때, 해당 메서드가 ***어느 타이밍에 호출***되는지 알아야함 
  - 템플릿 메서드의 로직 이해 필요 
- 로직의 변화가 생겨 상위 클래스 수정시 모든 서브 클래스 수정이 생김 
- 서브 클래스를 통해 기본 단계 (디폴트 단계 메서드)구현을 억제 
  - ***리스코프 치환 법칙을 위반할 여지***가 있다. 
## Visitor Pattern 
### 방문자 패턴이란? 
